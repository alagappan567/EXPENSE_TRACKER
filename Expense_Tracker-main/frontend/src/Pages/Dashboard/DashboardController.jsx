...
import { getUserBalanceApi } from "../../apis/user.api";

const DashboardController = () => {
  const [loading, setLoading] = useState(false);
  const [graphDuraction, setGraphDuraction] = useState(
    GRAPHICAL_VIEW_DURATION.WEEK_DAYS_DATA
  );
  const [pieChartDuraction, setPieChartDuraction] = useState(
    GRAPHICAL_VIEW_DURATION.WEEK_DAYS_DATA
  );
  ...
  const fetchTransaction = () => {
    ...
    dispatch(
      callApiAction(
        async () =>
          await getTrasactionApi({
            transactionFetchType: TRANSACTION_FETCH_TYPE.LIST_VIEW,
            listViewTime: LIST_VIEW_TIME.ALL,
            pageSize: 10,
          }),
        (response) => {
  ...
  const fetchTransactionForGraph = () => {
    dispatch(
      callApiAction(
        async () =>
          await getTrasactionApi({
            transactionFetchType: TRANSACTION_FETCH_TYPE.GRAPHICAL_VIEW,
            listViewTime: LIST_VIEW_TIME.ALL,
            graphicalViewType: GRAPHICAL_VIEW_TYPE.NO_FILTER,
            graphicalViewDuration: graphDuraction,
          }),
        (response) => {
    ...
    dispatch(
      callApiAction(
        async () =>
          await getTrasactionApi({
            transactionFetchType: TRANSACTION_FETCH_TYPE.GRAPHICAL_VIEW,
            graphicalViewType: GRAPHICAL_VIEW_TYPE.FILTER_BY_CATEGORY,
            graphicalViewDuration: pieChartDuraction,
          }),
        (response) => {
  ...
  useEffect(() => {
    if (graphDuraction == GRAPHICAL_VIEW_DURATION.WEEK_DAYS_DATA) {
      GraphData.map((row, index) => {
        setWeekGraphDayData((weekGraphDayData) => [
          ...weekGraphDayData,
          {
            amount: row.amount,
    ...
    if (graphDuraction == GRAPHICAL_VIEW_DURATION.MONTH_DAYS_DATA) {
      GraphData.map((row, index) => {
        setMonthGraphDayData((monthGraphDayData) => [
          ...monthGraphDayData,
          {
            amount: row.amount,
    ...
    if (graphDuraction == GRAPHICAL_VIEW_DURATION.MONTHLY_DATA) {
      GraphData.map((row, index) => {
        setYearGraphDayData((yearGraphDayData) => [
          ...yearGraphDayData,
          {
            amount: row.amount,
  ...
  return (
    <>
      <DashboardUI
        ...
        loading={loading}
        graphDuraction={graphDuraction}
        setGraphDuraction={setGraphDuraction}
        weekGraphDayData={weekGraphDayData}
        monthGraphDayData={monthGraphDayData}
        yearGraphDayData={yearGraphDayData}
        pieChartDuraction={pieChartDuraction}
        setPieChartDuraction={setPieChartDuraction}
